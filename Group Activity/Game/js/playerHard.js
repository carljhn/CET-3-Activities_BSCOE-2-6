...
import { CollisionExplosion } from "./collisionExplosion.js";
import { CollisionBird } from "./collisionBird.js";
import { CollisionBonus } from "./collisionBonus.js";
    ...
    checkCollision(){
        ...
        this.game.flying.forEach(flyings => {
            if (
                ...
                this.sound.src = "audio/bird-explosion.mp3";
                this.sound.play();    
                this.sound.volume = 0.3;    
                this.game.collision.push(new CollisionBird(
                    this.game, 
                    flyings.x + flyings.width * 0.5, 
                    flyings.y + flyings.height * 0.5));
                this.game.score--;
                this.game.floatingScore.push(new FloatingScore("-1", flyings.x, flyings.y, 175, 50));
            }
            else {
        ...
        this.game.car.forEach(cars => {
            if (
                ...
                this.game.collision.push(new CollisionExplosion(
                    this.game, 
                    cars.x + cars.width * 0.5, 
                    cars.y + cars.height * 0.5));
                this.game.score -= 3;
                this.game.floatingScore.push(new FloatingScore("-3", cars.x, cars.y, 175, 50));
            }
            else {
        ...
        this.game.redcar.forEach(redcars => {
            if (
                redcars.x < this.x + this.width &&
                redcars.x + redcars.width > this.x &&
                redcars.y < this.y + this.height &&
                redcars.y + redcars.height > this.y
            ){
                redcars.markedForDeletion = true;
                this.game.collision.push(new CollisionExplosion(
                    this.game, 
                    redcars.x + redcars.width * 0.5, 
                    redcars.y + redcars.height * 0.5));
                this.game.score -= 3;
                this.game.floatingScore.push(new FloatingScore("-3", redcars.x, redcars.y, 175, 50));
            }
            else {
        ...
        // collision with bonus tokens (score will be added by 2 points)
        this.game.bonus.forEach(bonuses => {
            if (
                bonuses.x < this.x + this.width &&
                bonuses.x + bonuses.width > this.x &&
                bonuses.y < this.y + this.height &&
                bonuses.y + bonuses.height > this.y
            ){
                bonuses.markedForDeletion = true;
                this.sound = new Audio();
                this.sound.src = "audio/bonus.wav";
                this.sound.play();
                this.sound.volume = 0.3;
                this.game.collision.push(new CollisionBonus(
                    this.game, 
                    bonuses.x + bonuses.width * 0.5, 
                    bonuses.y + bonuses.height * 0.5));
                this.game.score += 2;
                this.game.floatingScore.push(new FloatingScore("+2", bonuses.x, bonuses.y, 175, 50));
            }
            else {
